...
lin spline bili som linearHypothesis i stedet for anova
# lav linearhyp i spg c) isf anova
** øverst p 9
plot lineær spline - predict præcis som i slides
spg 3 titel mangler ´ i estimer
set.seed( 345 ) galt med æ
p 19 åndre sig
...
# Alternative måde (kan bruges til at dokumentere også pakkeversioner til sidst)
packages <- c("survival", "prodlim", "timereg", "car", "epitools", "broom", "descr", "plyr", "tidyr", 
			  "tidyverse", "survminer", "tableone", "rms")
...
bl_bilirubin_cut_points <- quantile(filter(wr, dead == 1)$bilirubin, 1:2/3)
glimpse(wr)
wr <- wr %>% 
	mutate(event = factor(dead, levels = 0:1, labels = c("Censored", "Dead")),
		   time_to_event = months, # mere sigende navn ("months" er enheden)
		   treatment = factor(azathiop, levels = 0:1, labels = c("Placebo", "Azathioprine")),
		   bl_bilirubin = bilirubin, # igen mere informativt navn
		   log2_bl_bilirubin = log2(bilirubin),
		   bl_bilirubin_p25 = (bl_bilirubin - 31) * (bl_bilirubin > 31), # til lineære splines
		   bl_bilirubin_p50 = (bl_bilirubin - 54) * (bl_bilirubin > 54),
		   bl_bilirubin_p75 = (bl_bilirubin - 119.5) * (bl_bilirubin > 119.5),
		   bl_bilirubin_binned = cut(bl_bilirubin, c(0, bl_bilirubin_cut_points, max(bl_bilirubin)))) %>% 
	as_tibble() # giver pænere output i konsol (viser kun de første 10 rækker modsat data frames)
wr
summary(wr)
```
...
- Flere overkrydsninger (proportional hazards-antagelsen er helt sikkert ikke overholdt)
...
summary(cox1)
pretty_coef(cox1, exp) # hazard ratio med CI-grænser for effekten af azathioprin
...
- Skævheden i sig selv er ikke et problem (husk i regressioner er det ikke kovariaterne fordelinger der skal være normalfordelt, men derimod residualerne; mange går i vandet dér)
- Problemet med skæve fordelinger er derimod, at der kan være outliers, som får uforholdsmæssig stor betydning for estimaterne, fordi de trækker mere, end de burde. Dette kan føre til ustabile estimater: hvis én observation trækker rigtigt meget i estimaterne, ændre estimaterne meget, hvis denne ikke var blevet registreret (eller hvis den korrekte værdi var blevet registreret, hvis det viser sig at være en datafejl)
